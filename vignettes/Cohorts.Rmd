...
$$E(a, d) = \bar X \times F_\omega (a) \times F_S(t-d) \times F_T(t-d) \times F_K(t-d).$$
...
```{r fig.width=7, fig.height=4} 
#| echo: false
F <- make_ts_function(5/365, season_par=Sp, trend_par=Tp, shock_par=Kp, times = tt)
plot(tt/365, F(tt), type ="l", ylab = expression(F[t](t)==F[S](t)*F[T](t)*F[K](t)), xlab = "Time (in Years)")
```
